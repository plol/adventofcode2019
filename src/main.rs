...
mod advent_13 {
    ...
    impl Iterator for &mut GameInputAndRenderIterator {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            if let Some(prev) = &self.prev_game {
                let ball_now = find_ball_position(&uhhh);
                let paddle_now = find_paddle_position(&uhhh);
                let ball_then = find_ball_position(prev);
                println!("{:?} -> {:?}", ball_then, ball_now);
                println!("going {:?}", (paddle_now.0 - ball_now.0).signum());
                self.prev_game = Some(uhhh.clone());
...
fn main() {
    ...
    advent_13::main2(read_input("inputs/input13"));
